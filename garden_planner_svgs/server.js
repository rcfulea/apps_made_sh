...
const usersData = loadJSON(usersFile).users || [];
...
function saveCustomPlants(data) {
  ...
  fs.writeFileSync(customPlantsFile, JSON.stringify(toSave, null, 2), 'utf8');
}
...
function handleApi(req, res) {
  ...
  if (url.pathname === '/api/login' && method === 'POST') {
    ...
    req.on('end', () => {
      ...
      res.setHeader('Content-Type', 'application/json');
      res.end(JSON.stringify({ status: 'ok', username: user.username }));
    });
    return;
  }
